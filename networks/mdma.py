...
def invert(f: Callable, x: t.Tensor, lb: float, ub: float, n_bisect_iter: int = 35) -> t.Tensor:
...
class MDMA(nn.Module):
    ...
    def __init__(
        ...
        self.w_s = t.nn.ParameterList([nn.Parameter(self.w_std * t.randn(self.d, self.m, 1, self.r) + w_bias)])
        self.b_s = t.nn.ParameterList([nn.Parameter(self.b_std * t.randn(self.d, self.m, self.r) + b_bias)])
        self.a_s = t.nn.ParameterList([nn.Parameter(self.a_std * t.randn(self.d, self.m, self.r))])
        for _ in range(self.l - 2):
            self.w_s += [nn.Parameter(w_scale * t.randn(self.d, self.m, self.r, self.r) + w_bias)]
            self.b_s += [nn.Parameter(self.b_std * t.randn(self.d, self.m, self.r) + b_bias)]
            self.a_s += [nn.Parameter(self.a_std * t.randn(self.d, self.m, self.r))]

        self.w_s += [nn.Parameter(w_scale * t.randn(self.d, self.m, self.r, 1) + w_bias)]
        self.b_s += [nn.Parameter(self.b_std * t.randn(self.d, self.m, 1) + b_bias)]
        ...
        if self.use_HT:
            ...
            self.pool_layer = AvgPool1d(kernel_size=self.HT_poolsize, stride=self.HT_poolsize, ceil_mode=True)
        ...
        if self.mix_vars:
            ...
            for n_perm, perm in enumerate(perms):
                permuted_nonzero_inds += [[n_perm, perm[inds[0]], perm[inds[1]]] for inds in nonzero_inds]

            params = (1 / np.sqrt(self.n_mix_terms)) * t.randn(len(permuted_nonzero_inds))
            self.mix_params = nn.Parameter(t.sparse_coo_tensor(list(zip(*permuted_nonzero_inds)),params, size=(self.m, self.d, self.d)).to_dense())
    ...
    def phis(self, X: t.Tensor, inds: List[int] = ..., single_phi_per_point: bool = False, ks: t.Tensor = None) -> t.Tensor:
        ...
        if single_phi_per_point:
            ...
            sliced_ws = [w[inds, ...].gather(1, ks.unsqueeze(-1).expand(*([-1, -1] + list(w.shape[2:])))) for w in self.w_s]
            sliced_bs = [b[inds, ...].gather(1, ks.expand(*([-1, -1] + list(b.shape[2:])))) for b in self.b_s]
            sliced_as = [a[inds, ...].gather(1, ks.expand(*([-1, -1] + list(a.shape[2:])))) for a in self.a_s]
        else:
    ...
    def phidots(self, X: t.Tensor, inds: List[int] = ..., missing_data_mask: t.Tensor = None) -> t.Tensor:
        ...
        for w, b, a in zip(sliced_ws[:-1], sliced_bs[:-1], sliced_as):
            phis = t.einsum('mjik,jikl->mjil', phis, self.nonneg(w)) + b
            phidots = t.einsum('mjik,jikl,mjil->mjil', phidots, self.nonneg(w), (1 + t.tanh(a) * tanhdot(phis)))
            phis = phis + t.tanh(phis) * t.tanh(a)
        phis = t.einsum('mjik,jikl->mjil', phis, self.nonneg(sliced_ws[-1])) + sliced_bs[-1]
        phidots = t.einsum('mjik,jikl,mjil->mjil', phidots, self.nonneg(sliced_ws[-1]), sigmoiddot(phis))
        ...
        if missing_data_mask is not None:
            ...
            phidots = t.einsum('mj,mji->mji', missing_data_mask, phidots) + t.einsum('mj,mji->mji', 1 - missing_data_mask, t.ones_like(phidots))
    ...
    def likelihood(self, X: t.Tensor, inds: List[int] = ..., stabilize: bool = False, missing_data_mask: t.Tensor = None) -> t.Tensor: 
    ...
    def MERA_contraction(self, T, inds: List[int] = ...) -> t.Tensor:
        ...
        for a_s, a2_s, couplings in zip(self.a_HTs[:-1], self.a_MERAs, self.all_couplings):
            T = t.stack([t.stack([t.prod(T[:, coupling, :], dim=1), 
                                  T[:, coupling[0], :] * t.roll(T[:, coupling[1], :], 
                                                                shifts=1, 
                                                                dims=1)]) if len(coupling) == 2 else T[:, coupling, :].squeeze().unsqueeze(0).expand(2, -1, -1) for coupling in couplings])
            a_s = self.nonneg(a_s)
            a_s = a_s / t.sum(a_s, dim=1, keepdim=True)
    ...
    def log_density(self, X: t.Tensor, inds: List[int] = ..., eps: float = 1e-40, stabilize: bool = False, missing_data_mask: t.Tensor = None) -> t.Tensor:
        ...
        lk, fm = self.likelihood(X, inds, stabilize=stabilize, missing_data_mask=missing_data_mask)
        if stabilize:
            if inds is ...:
                n_vars = self.d
            else:
    ...
    def nll(self, X: t.Tensor, inds: List[int] = ..., stabilize: bool = False, missing_data_mask: t.Tensor = None) -> t.Tensor:
        ...
        return -t.mean(self.log_density(X, inds=inds, stabilize=stabilize, missing_data_mask=missing_data_mask))

    def sample_autoregressive(self, S: int, batch_size: int = None, inds: List[int] = None, n_bisect_iter: int = 35, upper_bound: float = 1e3, lower_bound: float = -1e3, eps: float = 1e-10) -> t.Tensor:
        ...
        with t.no_grad():
            ...
            U_dataloader = t.utils.data.DataLoader(U, batch_size=batch_size, shuffle=False)
            for batch_idx, batch in enumerate(U_dataloader):
                ...
                for k in range(dim_samples):
                    ...
                    samples[:, k] = invert(curr_condCDF, batch[:, k], n_bisect_iter=n_bisect_iter, ub=upper_bound, lb=lower_bound)
                all_samples.append(samples.cpu().detach().numpy())
            return np.concatenate(all_samples)

    def sample(self, S: int, batch_size: int = None, inds: List[int] = ..., cond_inds: List[int] = None, cond_X: t.Tensor = None, n_bisect_iter: int = 35, upper_bound: float = 1e3, lower_bound: float = -1e3) -> t.Tensor:
        ...
        with t.no_grad():
            ...
            for a, ff in zip(reversed(self.a_HTs), reversed(ffs)):
                ...
                a = self.nonneg(a.permute(1, 0, 2))  # multinomial_coeffs[p,q,s] = a[p,q,ks[q,s]]
                multinomial_coeffs = a.gather(2, ks.unsqueeze(0).expand(a.shape[0], -1, -1))
                ...
                new_ks = t.multinomial(multinomial_coeffs, 1).reshape_as(ks)  # dim_{l+1} x S
                ks = t.repeat_interleave(new_ks, repeats=self.HT_poolsize, dim=0)  # (self.HT_poolsize*dim_{l+1}) x S
            ...
            U_dataloader = t.utils.data.DataLoader(t.stack([U, ks]).transpose(0, 1), batch_size=batch_size, shuffle=False)
            for batch_idx, batch in enumerate(U_dataloader):
                batch_Us, batch_ks = batch[:, 0, :], batch[:, 1, :].long()

                # Define a CDF function R^(batch_size x self.d) -> [0,1]^(batch_size x self.d) and invert
                def CDF(s):
                    phis = self.phis(s, inds=inds, single_phi_per_point=True, ks=batch_ks)
                    return phis.squeeze().t()

                samples = invert(CDF, batch_Us, n_bisect_iter=n_bisect_iter, ub=upper_bound, lb=lower_bound)
                all_samples.append(samples.cpu().detach().numpy())
            return np.concatenate(all_samples).reshape(
                    (n_conds, S, n_vars)).squeeze()

    def conditional_features(self, cond_inds: List[int], cond_X: t.Tensor):
        """
    ...
    def condCDF(self, k: int, prev_samples: t.Tensor, inds: List[int], eps=1e-10) -> Callable[[t.Tensor], t.Tensor]:
    ...
    def cond_density(self, X: t.Tensor, inds: List[int], cond_X: t.Tensor, cond_inds: List[int]) -> t.Tensor:
    ...
    def create_default_couplings(self) -> List[List[List[int]]]:
        ...
        for _ in range(self.L_HT):
            coupling_ranges = [(j, min(dim_l, j + self.HT_poolsize)) for j in range(0, dim_l, self.HT_poolsize)]
    ...
    def create_adaptive_couplings(self, batches: List[t.Tensor]):
        ...
        with t.no_grad():
            pool_layer = AvgPool2d(kernel_size=self.HT_poolsize, stride=self.HT_poolsize, ceil_mode=True)
            ...
            for _ in range(self.L_HT):
                ...
                if n_couples > 1:
                    ...
                    sorted_flat_inds = sorted_flat_corrs.indices[::2]  # discard symmetric entries
                    sorted_inds = [[ind // dim_l, ind % dim_l] for ind in sorted_flat_inds]
    ...
    def mix_X(self, X: t.Tensor) -> t.Tensor:
        ...
        X = X + t.einsum('ijk,bkil->bjil', self.nonneg(self.mix_params), t.sigmoid(X))
